(function (global) {
  ...
  callAPI().then(dataSQL => { AddressBook.prototype.data = dataSQL })


  var AddressBook = function (name, phoneNumber, email) {
    return new AddressBook.init(name, phoneNumber, email);
  };
  ...
  AddressBook.init = function (name, phoneNumber, email) {
    var self = this;
    //set up the address book
    if (name || phoneNumber || email) {
      self.addNewContact(name || "", phoneNumber || "", email || "");
    }
...
form.addEventListener('submit', function () {
  if (!window.contactList) { //check if we already have a contact list
    window.contactList = $ab(form.person.value, form.phoneNumber.value, form.email.value);
  } else {
    //saves new values rather than deleting old ones as well
    contactList.addNewContact(form.person.value, form.phoneNumber.value, form.email.value);
  }
...
searchForm.addEventListener('submit', function () {
  ...
  if (results.length > 0) {
    ...
    for (var i = 0; i < results.length; i++) {
      document.getElementById('results').innerHTML += '<div class="contact-item">Name:' + results[i].name + '<br>Phone:' + results[i].phoneNumber + '<br>Email:' + results[i].email + '</div><hr>';
    }
  } else {
  ...
  if (window.contactList) { //check if we already have a contact list
    ...
    if (contacts.length > 0) {
      for (var i = 0; i < contacts.length; i++) {
        document.getElementById('show-panel').innerHTML += '<div class="contact-item">Name:' + contacts[i].name + '<br>Phone:' + contacts[i].phoneNumber + '<br>Email:' + contacts[i].email + '</div><hr>';
      }
    } else {
